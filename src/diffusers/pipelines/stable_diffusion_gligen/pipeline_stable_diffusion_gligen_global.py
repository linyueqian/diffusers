...
class StableDiffusionGLIGENGlobalPipeline(DiffusionPipeline, StableDiffusionMixin):
    r"""
    ...
    def __call__(
        ...
        clip_skip: Optional[int] = None,
        first_conv_state_dict: Optional[Dict[str, torch.Tensor]] = None,
    ):
        r"""
        ...
        if gligen_scheduled_sampling_beta < 1:
            assert first_conv_state_dict is not None, \
                "When using scheduled sampling, you need to provide the first conv state dict."
        ...
        masks = torch.ones(len(gligen_global_image), 1).to(device=device, dtype=self.unet.dtype)

        repeat_batch = batch_size * num_images_per_prompt
        assert repeat_batch == 1, "Not sure what to do when repeat_batch > 1"
        ...
        cross_attention_kwargs["gligen"] = {"conditions": gligen_global_image, "masks": masks}
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == num_grounding_steps:
                    ...
                    self.unet.restore_first_conv(first_conv_state_dict)
                ...
                noise_pred = self.unet(
                    ...
                    grounding_extra_input=gligen_global_image
                ).sample
